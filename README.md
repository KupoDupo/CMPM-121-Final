...
4. External DSL:
We created an external domain-specific language (DSL) to define game levels and puzzles in a more human-readable format. The DSL allows us to specify room layouts, object placements, and puzzle logic using a simple syntax. We then wrote a parser in Lua that reads the DSL files and translates them into game objects and behaviors within the LÖVE engine. This approach makes it easier to design and modify game content without directly editing the Lua code, allowing for faster iteration and prototyping. The DSL files are stored separately from the main game code, making it easy to add new levels or modify existing ones by simply updating the DSL files.
